...
######################################################################################
#                                                                                    #
# THIS PROGRAM IS TO IDENTIFY ALL THE ELASTIC INDICES WHICH ARE OLDER THAN 7 DAYS  & #
# OPENED. IF FOUND THEN DELETE THEM FROM THE ELASTICSEARCH CLUSTER.                  #
# V. 1.0                                                                             #
#                                                                                    #
######################################################################################
...
# functions to manipulate the indices on elasticsearch
...
def filter_indices_to_delete(open_indices_list):
   ...
   # print("one month old date: %s" % one_months_old_datetime)
    ...
    print ("************************** Calculated Indices Delta Datetime ************************")
...
def get_closed_indices_list(indics_file=None):
    ...
    indices = list()
    if indics_file:
        print(" ************** List of all the open indices **************")
...
def main():
    elastic_cluster_url = 'http://<hostname or IP address>:9200/'
    ...
    print("********************* Filtered indices(older than 7 days) list tobe deleted ********************")
    for i in indices_to_delete:
        print(i)

    # Delete the indices older than 7 days
